...
socket.on('qr_code', ({ phone, qr }) => {
  $('qr-display').style.display = 'block';
  $('code-display').style.display = 'none';
  $('qr-img').src = qr;
  setPairingStatus(`Scan QR Code untuk nomor ${phone}`, 'loading');
  addLog(`QR Code baru untuk ${phone} — scan sekarang!`, 'info');
});

socket.on('qr_error', ({ phone, message }) => {
  setPairingStatus(`Error: ${escHtml(message)}`, 'error');
  addLog(`QR error ${phone}: ${message}`, 'error');
});
...
// Active pairing method: 'pairing' or 'qr'
let pairingMethod = 'pairing';

function setupPairingPage() {
  // Method toggle buttons
  document.querySelectorAll('.method-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      pairingMethod = btn.dataset.method;

      document.querySelectorAll('.method-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');

      // Show/hide instructions
      $('instructions-pairing').style.display = pairingMethod === 'pairing' ? 'block' : 'none';
      $('instructions-qr').style.display      = pairingMethod === 'qr'      ? 'block' : 'none';
  ...
  $('req-connect').addEventListener('click', requestConnect);
  $('pairing-phone').addEventListener('keydown', e => {
    if (e.key === 'Enter') requestConnect();
  });
}
...
function requestConnect() {
  ...
  if (!phone || phone.length < 8 || phone.length > 15) {
    setPairingStatus('Masukkan nomor yang valid (8-15 digit)', 'error');
    return;
  }
  ...
  setTimeout(() => { $('req-connect').disabled = false; }, 20000);

  if (pairingMethod === 'pairing') {
    setPairingStatus('Meminta pairing code...', 'loading');
    socket.emit('request_pairing', { phone });
  } else {
    setPairingStatus('Memuat QR Code...', 'loading');
    socket.emit('request_qr', { phone });
  }
}
