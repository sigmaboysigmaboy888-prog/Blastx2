...
async function createSession(phone, method, socket) {
  ...
  waSocket.ev.on('connection.update', async (update) => {
    ...
    // ── Pairing Code ─────────────────────────────────────────────────────────
    // Baileys official: requestPairingCode dipanggil setelah socket open
    // dan creds belum registered
    if (
      method === 'pairing' &&
      connection === 'open' &&
      ...
      try {
        ...
        console.error(`[PairingCode] ${phone}:`, err.message);
        socket.emit('pairing_error', { phone, message: `Gagal: ${err.message}` });
      }
    }
