...
async function createSession(phone, method, socket) {
  ...
  // Pairing code: JANGAN pakai mobile:true (deprecated)
  // Cukup makeWASocket biasa, lalu requestPairingCode() setelah connecting
  ...
  waSocket.ev.on('connection.update', async (update) => {
    ...
    // ── QR Code: emit setiap kali QR baru muncul ────────────────────────────
    if (qr && method === 'qr') {
      try {
        ...
        log(socket, `QR Code siap untuk ${phone} — scan sekarang!`, 'info');
      } catch (err) {
        console.error('[QR generate]', err.message);
      }
    }

    // ── Pairing Code: request saat WS connecting (bukan open) ───────────────
    // Baileys terbaru: requestPairingCode dipanggil SEBELUM connected
    // dan HANYA jika belum registered
    if (
      method === 'pairing' &&
      ...
      // Delay singkat agar WS socket siap
      reconnectTimers[`pair_${phone}`] = setTimeout(async () => {
        ...
        while (attempts < maxAttempts) {
          ...
          try {
            ...
            if (!code) throw new Error('Kode null dari server WA');
            ...
            if (attempts >= maxAttempts) {
              socket.emit('pairing_error', {
                phone,
                message: `Gagal pairing code: ${err.message}`,
              });
            } else {
              await sleep(3000);
            }
      ...
      }, 1500); // 1.5s — cukup untuk WS init tanpa mobile:true
    }
